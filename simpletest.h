...
#define TEST(require, expression, format, ...)                                                     \
    do                                                                                             \
    {                                                                                              \
        unsigned tick = simpletest_gettick();                                                      \
        int result = simpletest_test(expression);                                                  \
        tick = simpletest_gettick() - tick;                                                        \
        if(!result)                                                                                \
        {                                                                                          \
            simpletest_warn("  [%d] %s in %s:%s:%d cost %0.3f ms\n" format "\n",                   \
        ...
        {                                                                                          \
            simpletest_debug("  [%d] PASSED in %s:%s:%d cost %0.3f ms\n" format "\n",              \
                             simpletest_count(), __FILE__, __FUNCTION__, __LINE__, tick / 1000.,   \
                             ##__VA_ARGS__);                                                       \
        }                                                                                          \
    } while(0)
...
#define SIMPLETEST_ARG1N(x, n, ...) __VA_ARGS__
#define SIMPLETEST_ARG0(x, n, ...) n
...
#define EXPECT(expression, ...)                                                                    \
    TEST(0, (expression),                                                                          \
         "  ==>  %s\n"                                                                             \
         "  ==>  %s\n  " SIMPLETEST_ARG0(0, ##__VA_ARGS__, ""),                                    \
         #expression, (result ? "true" : "false"), SIMPLETEST_ARG1N(0, ##__VA_ARGS__, 0, 0))
/**
 * @brief 要求表达式结果为真
 * @param expression 表达式
 * @param ... 额外输出信息
 ...
 * @param actual 实际表达式
 */
#define EXPECT_EQ_STR(expect, actual) TEST_OP(0, simpletest_eq_str, expect, actual, "\"%s\"")
/**
 ...
 * @param actual 实际表达式
 */
#define REQUIRE_EQ_STR(expect, actual) TEST_OP(1, simpletest_eq_str, expect, actual, "\"%s\"")
/**
...
#define EXPECT_EQ_MEM(expect, actual, len)                                                         \
    TEST_OPA(1, simpletest_eq_mem, expect, actual, "0x%08X", len)
/**
 * @brief 要求2块内存相等
 * @param expect 期望表达式
 * @param actual 实际表达式
